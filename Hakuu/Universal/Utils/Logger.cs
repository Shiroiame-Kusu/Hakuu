    ...
    internal static class Logger
    {
        ...
        public static void Output(LogType type, params object?[] objects)
        {
            ...
            if (!typeName.Contains("Notice") && (
                typeName.StartsWith("Bot_") && !Global.Settings.Hakuu.PagesDisplayed.Bot ||
                typeName.StartsWith("Plugin_") && !Global.Settings.Hakuu.PagesDisplayed.JSPlugin ||
                typeName.StartsWith("Server_") && !Global.Settings.Hakuu.PagesDisplayed.ServerPanel
            ))
            {
                return;
            }
            ...
            switch (type)
            {
                ...
                case LogType.Version_New:
                    WriteLine(1, $"当前版本：{Global.VERSION} （发现新版本:{line}{(Global.Settings.Hakuu.AutoUpdate ? "，下载任务已在后台开始" : string.Empty)}）", true);
                    break;
                ...
                case LogType.Server_Output:
                    ...
                    break;
                case LogType.Server_Notice:
                    Program.Ui.ServerPanelConsoleWebBrowser_Invoke("<span style=\"color:#4B738D;font-weight: bold;\">[Hakuu]</span>" + LogPreProcessing.EscapeLog(line));
                    break;
                case LogType.Server_Clear:
                    Program.Ui.ServerPanelConsoleWebBrowser_Invoke("#clear");
                    break;
                case LogType.Bot_Notice:
                    Program.Ui.BotWebBrowser_Invoke("<span style=\"color:#4B738D;font-weight: bold;\">[Hakuu]</span>" + LogPreProcessing.EscapeLog(line));
                    break;
                case LogType.Bot_Receive:
                ...
                case LogType.Plugin_Info:
                    ...
                    break;
                case LogType.Plugin_Notice:
                    Program.Ui.JSPluginWebBrowser_Invoke("<span style=\"color:#4B738D;font-weight: bold;\">[Hakuu]</span>" + LogPreProcessing.EscapeLog(line));
                    break;
                case LogType.Plugin_Error:
                ...
                case LogType.Version_New:
                    ...
                    Program.Ui.SettingHakuuVersion_Update($"当前版本：{Global.VERSION} （发现新版本:{line}，你可以{(Global.Settings.Hakuu.AutoUpdate ? "等待后台自动下载或手动" : string.Empty)}点击下方链接获取最新版）");
                    break;
                case LogType.Version_Latest:
                    Program.Ui.ShowBalloonTip(
                        "获取更新成功\n" +
                        "当前已是最新版:)");
                    Program.Ui.SettingHakuuVersion_Update($"当前版本：{Global.VERSION} （已是最新版）");
                    break;
                case LogType.Version_Failure:
                case LogType.Version_DownloadFailed:
                ...
                case LogType.Server_Notice:
                    ...
                    if (Catalog.Server.Cache.Count > Global.Settings.Hakuu.MaxCacheLines)
                    {
                        Catalog.Server.Cache.RemoveRange(0, Catalog.Server.Cache.Count - Global.Settings.Hakuu.MaxCacheLines);
                    }
                    break;
                case LogType.Server_Clear:
                ...
                case LogType.Version_New:
                    Catalog.Notification?.Show(
                        "Hakuu",
                        "发现新版本：" + line + "\n" + (Global.Settings.Hakuu.AutoUpdate ? "你可以等待后台自动下载或手动点击本提示打开下载页面" : "点击本提示可打开下载页面"),
                        onClick: () => Process.Start(
                            new ProcessStartInfo("https://github.com/Zaitonn/Hakuu/releases/latest") { UseShellExecute = true }
                            ),
                        expirationTime: TimeSpan.FromSeconds(10)
                        );
                    Catalog.Settings.Hakuu?.UpdateVersion($"\n（发现新版本:{line}，你可以{(Global.Settings.Hakuu.AutoUpdate ? "等待后台自动下载或手动" : string.Empty)}点击下方链接获取最新版）");
                    break;
                case LogType.Version_Latest:
                    Catalog.Notification?.Show(
                        "Hakuu",
                        "获取更新成功\n" +
                        "当前已是最新版:)");
                    Catalog.Settings.Hakuu?.UpdateVersion("（已是最新版）");
                    break;
                case LogType.Version_Failure:
                case LogType.Version_DownloadFailed:
                    Catalog.Notification?.Show("Hakuu", $"更新{(type == LogType.Version_Failure ? "获取" : "下载")}异常：\n" + line);
                    break;
                case LogType.Version_Ready:
                    Catalog.Notification?.Show("Hakuu", line, expirationTime: TimeSpan.FromSeconds(10));
                    break;
#endif
                default:
#if !CONSOLE
                    if (!Global.Settings.Hakuu.DevelopmentTool.EnableDebug ||
                        !Global.Settings.Hakuu.DevelopmentTool.DetailDebug && type == LogType.DetailDebug)
                    {
                        return;
                    }
#endif
                    ...
                    line =
                        ...
                        $"{(Global.Settings.Hakuu.DevelopmentTool.DetailDebug ? " " + stackTrace.GetFrame(1)!.GetMethod() : "." + stackTrace.GetFrame(1)!.GetMethod()!.Name)}] " +
                        $"{line}";
#if CONSOLE
                    if (!(!Global.Settings.Hakuu.DevelopmentTool.EnableDebug ||
                        !Global.Settings.Hakuu.DevelopmentTool.DetailDebug && type == LogType.DetailDebug))
        ...
        /// <param name="usingTitle">使用Hakuu标题</param>
        private static void WriteLine(int level, string line, bool usingTitle = false)
        {
            ...
            lock (_lock)
            {
                ...
                if (!Global.Settings.Hakuu.ColorfulLog)
                {
