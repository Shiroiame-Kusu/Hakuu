...
namespace Hakuu.Core.JSPlugin
{
    internal static class JSPluginManager
    {
        ...
        private static void SummaryAll()
        {
            ...
            {
                Utils.Logger.Output(LogType.Plugin_Notice, $"插件加载完毕，但是并没有插件被加载。扩展市场：https://market.Hakuu.cc/");
            }
        }
        ...
        private static void LoadFile(string file)
        {
            ...
            if (Global.Settings.Hakuu.Function.JSPatternToSkipLoadingSpecifiedFile.Where((pattern) => lowerFile.EndsWith(pattern.ToLowerInvariant())).Count() != 0)
            {
                return;
            }
