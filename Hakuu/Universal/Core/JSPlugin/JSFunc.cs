    ...
    internal static class JSFunc
    {
        ...
        public static bool Trigger(EventType type, params object?[] args)
        {
            ...
            lock (_eventLock)
            {
                Logger.Output(LogType.Debug, type);
                {
                    ...
                    if (tasks.Count > 0)
                    {
                        if (Global.Settings.Hakuu.Function.JSEventMaxWaitingTime > 0)
                        {
                            Task.WaitAll(tasks.ToArray(), Global.Settings.Hakuu.Function.JSEventMaxWaitingTime);
                            tokenSource.Cancel();
                            tasks.Select((task) => task.IsCompleted && task.Result).ToList().ForEach((result) => interdicted = interdicted || result);
                        }
                        Global.Settings.Hakuu.Function.JSEventCoolingDownTime.ToSleep();
                    }
                }
            }
            return interdicted;
        ...
        public static bool ReloadFiles(string? @namespace, string type)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(@namespace))
                {
                    Logger.Output(LogType.Plugin_Warn, $"[{@namespace}]", $"重新加载了Hakuu的{(type ?? string.Empty).ToLowerInvariant()}文件");
                }
                return true;
            }
            catch (Exception e)
