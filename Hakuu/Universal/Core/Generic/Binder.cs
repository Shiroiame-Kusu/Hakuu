...
namespace Hakuu.Core.Generic
{
    internal static class Binder
    {
        ...
        public static void Bind(Message message, string? value)
        {
            ...
            if (!System.Text.RegularExpressions.Regex.IsMatch(value, Global.Settings.Hakuu.Function.RegexForCheckingGameID))
            {
                EventTrigger.Trigger(EventType.BindingFailDueToInvalid, message);
                return;
            }
            if (Global.Settings.Hakuu.Function.DisableBinderWhenServerClosed ? !ServerManager.Status : false)
            {
