...
namespace Hakuu.Ui.ChildrenWindow
{
    public partial class ScheduleEditor : Form
    {
        ...
        private void Confirm_Click(object sender, EventArgs e)
        {
            ...
            {
                if (Core.Generic.Command.GetType(Command.Text) == Base.CommandType.Invalid)
                {
                    MessageBox.Show("执行命令无效", "Hakuu", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                ...
                else
                {
                    ...
                    MessageBox.Show("Cron表达式无效", "Hakuu", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else
            {
                MessageBox.Show("内容为空", "Hakuu", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        {
            Process.Start(new ProcessStartInfo("https://Hakuu.cc/docs/guide/schedule") { UseShellExecute = true });
        }
