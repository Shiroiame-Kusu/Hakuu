...
namespace Hakuu.Windows
{
    public partial class MainWindow : UiWindow
    {
        public MainWindow()
        {
            ...
            SettingsNavigationItem.Visibility = Global.Settings.Hakuu.PagesDisplayed.Settings ? Visibility.Visible : Visibility.Hidden;
            if (!Global.Settings.Hakuu.PagesDisplayed.ServerPanel && !Global.Settings.Hakuu.PagesDisplayed.ServerPluginManager)
            {
                ServerNavigationItem.Visibility = Visibility.Hidden;
            }
            if (
                !Global.Settings.Hakuu.PagesDisplayed.Bot &&
                !Global.Settings.Hakuu.PagesDisplayed.Member &&
                !Global.Settings.Hakuu.PagesDisplayed.RegexList &&
                !Global.Settings.Hakuu.PagesDisplayed.Schedule &&
                !Global.Settings.Hakuu.PagesDisplayed.JSPlugin)
            {
        ...
        private void UiWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (Global.Settings.Hakuu.ThemeFollowSystem)
            {
        ...
        private void UiWindow_Closing(object sender, CancelEventArgs e)
        {
            if (ServerManager.Status)
            {
                ...
                ShowInTaskbar = false;
                Hide();
                Catalog.Notification?.Show("Hakuu", "服务器进程仍在运行中\n已自动最小化至托盘，点击托盘图标即可复原窗口");
            }
        }
        ...
        public void UpdateTitle(string? title)
            => Dispatcher.Invoke(() => _TitleBar.Title = string.IsNullOrEmpty(title) ? "Hakuu" : $"Hakuu - {title}");
    }
}
