            ...
            if (sender is Wpf.Ui.Controls.MenuItem menuItem && menuItem != null)
            {
                ...
                switch (Tag)
                {
                    ...
                    case "Edit":
                        ...
                        break;
                    case "Delete":
                        if (Logger.MsgBox("确定删除此行数据？\n它将会永远失去！（真的很久！）", "Hakuu", 1, 48) && ScheduleListView.SelectedIndex >= 0)
                        {
                        ...
                        break;
                    case "Clear":
                        if (Logger.MsgBox("确定删除所有数据？\n它将会永远失去！（真的很久！）", "Hakuu", 1, 48) && ScheduleListView.Items.Count > 0)
                        {
                            ScheduleListView.Items.Clear();
                            Save();
                        }
                    ...
                    case "Refresh":
                        ...
                        break;
                    case "LookupCommand":
                        Process.Start(new ProcessStartInfo("https://Hakuu.cc/docs/guide/command") { UseShellExecute = true });
                        break;
                    case "LookupVariables":
                        Process.Start(new ProcessStartInfo("https://Hakuu.cc/docs/guide/variables") { UseShellExecute = true });
                        break;
                    case "Enable":
                    case "Disable":
                        if (selectedItem != null)
